...
function totaljournererror($date) {
    include ("include/configuration.php");

    $list = array();
    $sql = "SELECT employes.id as id,usagers.image as image, CONCAT(employes.prenom, ' ', employes.nom) as nom, punch.moment as temp FROM punch 
INNER JOIN employes ON punch.employes_id = employes.id 
INNER JOIN usagers ON employes.id = usagers.employes_id
WHERE punch.moment LIKE '$date%' order by punch.id asc;";
            ...
            if (count($data) % 2 == 0) {
                ...
                if (count($data) === 4){
                    ...
                    if ($total2>3600){
                        ...
                        echo "<span class=\"badge badge-warning badge-pill ml-auto\">Pause déplacée de $hours2 h : $minutes2 m</span>";
                    }
